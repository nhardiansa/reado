...
| `src/components/`      | Shared claymorphism UI system (`ClayCard`, `ClayButton`, `ClayModal`, etc.). Reusable across features.                                                                                   | `theme/`                                                        |
...
Route (src/app/<route>.tsx)
  └─ mounts Feature Screen (src/features/<name>/screens/)
       ...
       └─ renders Shared Components (src/components/)    ← claymorphism system
...
6. **`components/` is the design system.** Only claymorphism primitives and generic patterns (EmptyState, ErrorState). Feature-specific visuals live in the feature.
...
| `components/`   | RNTL                               | Claymorphism primitives render tokens correctly, accept className/style overrides.                         |
